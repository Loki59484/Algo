...
class CFOTracker(App):
    ...
    def zmq_listener(self):
        ...
        # Connect to your AWS Tailscale IPs
        sub_socket.connect(f"tcp://{AWS_TAILSCALE_IP}:5556")
        sub_socket.connect(f"tcp://{AWS_TAILSCALE_IP}:5557")
        sub_socket.setsockopt_string(zmq.SUBSCRIBE, "")    
        
        while True:
            try:
                # This will wait until a message arrives
                message = sub_socket.recv_string(flags=zmq.NOBLOCK)
                
                # Assuming your scripts send something like "PING:Live Trader"
                if message.startswith("PING:"):
                    component_name = message.split(":")[1]
                    # Instantly update the shared dictionary with the current time
                    self.incoming_data[component_name] = time.time()
                    
            except zmq.Again:
                # No message received, sleep briefly to prevent CPU hogging
                time.sleep(0.1)
            except Exception as e:
                self.query_one(RichLog).write(f"[red]ZMQ Error: {e}[/red]")
