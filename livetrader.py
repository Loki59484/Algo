...
def place_buy_order(
    ...
    try:
        ...
        logger.info(
            f"[{gear_name}] MARKET ORDER FILLED: Buy {opt_leg.type} @ ₹{buy_price:.2f} (Base LTP was ₹{close_price:.2f}) # Qty {qty}"
        )
        return 1
...
def _manage_position(
    bucket, call_option, ce_df, pe_df, timestamp, active_params, gear_name
):
    try:
        ...
        pos_key = getattr(
            pos,
            "instrument_token",
            getattr(pos, "instrument_key", getattr(pos, "key", None)),
        )
        ...
        if target_hit or stoploss_hit or eod_square_off:
            ...
            bucket.pending_exit = {"remark": remark, "gear": gear_name}
            logger.info(
                f"[{gear_name}] {remark} TRIGGERED : Routing Market Order to exchange... (Simulating 1-Tick Latency)"
            )
        else:
...
async def execute_live(
    ...
    **kwargs,
    ...
    # ---------------------------------------------------------
    # 1. MANAGE CURRENT OPEN POSITIONS (INSTANT EXITS)
    # ---------------------------------------------------------
...
def _manage_position_live(
    ...
    """Live version of position manager that fires exchange orders immediately."""
    try:
        pos = bucket.open_position
        pos_key = getattr(
            pos,
        ...
        # Initialize Trailing SL and Targets
        if not hasattr(pos, "highest_seen"):
            pos.highest_seen = close
            atr = active_row.get("ATRr_14", 1.0)
            pos.trail_dist = active_params.get("trailing_sl_atr", 1.5) * atr
        ...
        if target_hit or stoploss_hit or eod_square_off:
            remark = "TARGET" if target_hit else "STOPLOSS" if stoploss_hit else "EOD"
            logger.info(
                f"[{gear_name}] {remark} TRIGGERED : Routing Market Sell Order INSTANTLY..."
            )

            last_trade = trader.portfolio.report[-1]
            qty_to_sell = getattr(
                last_trade, "buy_qty", getattr(last_trade, "Buy_qty", 0)
            )

            # Fire the sell order directly to the broker (DO NOT delay)
            status = trader.broker.sell_order(
                key=active_opt.key, qty=qty_to_sell, price=close
            )

            if status != -1:
                # In live trading, extract the actual API fill price here if your broker object returns it!
                # For now, falling back to current LTP.
                actual_fill_price = (
                    status.get("average_price", close)
                    if isinstance(status, dict)
                    else close
                )
                _close_trade(
                    trader, bucket, timestamp, actual_fill_price, remark, gear_name
                )
...
async def executor(
    ...
    **kwargs,
    ...
    if not hasattr(bucket, "pending_entry"):
        bucket.pending_entry = None
    if not hasattr(bucket, "pending_exit"):
    ...
    if bucket.open_position is not None and bucket.pending_exit is not None:
        ...
        status = trader.broker.sell_order(
            key=active_opt.key, qty=qty_to_sell, price=exit_price
        )
        if status != -1:
    ...
    if ce_cond:
        ...
        logger.info(
            f"[{gear_name}] SIGNAL FIRED (CE): Routing to exchange... (Simulating 1-Tick Latency)"
        )
    elif pe_cond:
        bucket.pending_entry = {
            "type": "PE",
        ...
        logger.info(
            f"[{gear_name}] SIGNAL FIRED (PE): Routing to exchange... (Simulating 1-Tick Latency)"
        )
...
async def processor(
    ...
    **kwargs,
    ...
    spot_key = next(
        (
            k
            for k in trader.instruments.keys()
            if "INDEX" in k[0] or trader.instruments[k].type == "SPOT"
        ),
        None,
    )
    ...
    while not stopevent.is_set():
        try:
            ticks: dict[str, Tick] = await asyncio.wait_for(
                trader.datafeed.get(), timeout=0.5
            )
        ...
        try:
            ...
            for key, val in trader.instruments.items():
                if key[0] in trading_items:
                    tasks.append(
                        asyncio.to_thread(
                            trader.strategy.apply, val.historical_candles, key=key
                        )
                    )
...
def _get_best_options(insts, market_quote):
    ...
    for _, quote in market_quote.items():
        opt_type = insts.loc[
            insts["instrument_key"] == quote["instrument_token"], "instrument_type"
        ].item()
...
def _setup_sim_dummy(spot, trading_date, spot_file, ce_file, pe_file):
    ...
    options = insts[
        (insts["instrument_key"] == best_ce) | (insts["instrument_key"] == best_pe)
    ]
    parsed_insts = []
...
def _setup_live(spot: str | list[str], trading_date: date):
    ...
    parsed_insts = Instrument.parse_options(
        client=ustox, options=options, lookback=2, is_expired=False
    )
...
def setup_mode(args, trader: ana.Trader):
    ...
    if args.command == "live":
        ...
        if args.tickwise == "dummy":
            insts = _setup_sim_dummy(
                args.index, trading_date, args.call, args.put, args.spot
            )
        else:
...
async def starter(trader, args, tradable_insts, feeder_queue):
    # 1. Pre-define variables so the 'finally' block never encounters an UnboundLocalError
    tasks_to_run = []
    ui_socket = None
    context = None
    ...
    try:
        ...
        tasks.append(
            processor(trader, trader.buckets[-1], stopevent, ui_socket=ui_socket)
        )
        ...
        # 2. Safely cancel tasks
        for task in tasks_to_run:
            if not task.done():
                task.cancel()
                logger.info(f"Cancelled async task: {task.get_coro()}.")
                
        # 3. Safely close sockets only if they were successfully created
        if args.tui:
            if ui_socket:
                ui_socket.close()
            if context:
